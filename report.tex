...
We assume 10\,Gbps per link (standard DC access speed).
For fat trees: $B = k^d \times 10$ Gbps (each of $k^{d-1}$ root switches has $k$ links per side).
For other topologies: we enumerate all balanced host-group partitions via bitmask (feasible for $\leq$20 groups), greedily assign switches to minimise crossing links, and return the minimum cut $\times\,10$\,Gbps.
...
\noindent\textbf{M1 $\to$ M3:} \textbf{2} paths (length 4). Same pod, different edge switches; 2 level-1 switches provide 2 disjoint up--down paths.

\noindent\textbf{M1 $\to$ M5:} \textbf{4} paths (length 6). Different pods; must traverse root level. $2$ choices at level-1 $\times$ $2$ choices at root $= 2^2 = 4$.

\noindent\textbf{M1 $\to$ M5 via S2:} \textbf{1} path.  S2 is one of 4 root switches. By product principle: $\mathrm{count}(M1 \!\to\! S2) \times \mathrm{count}(S2 \!\to\! M5) = 1 \times 1 = 1$.

\subsection{Part (c): Bisection Bandwidth}
\[
  B = k^d \times 10 = 2^3 \times 10 = \mathbf{80} \text{ Gbps}
\]
The minimum cut is at the root level: 4 root switches $\times$ 2 links each $\times$ 10\,Gbps.
...
\textbf{(c)} Bisection BW $= k^d \times 10 = 4^3 \times 10 = \mathbf{640}$ Gbps.
...
\textbf{(c)} Bisection BW $= 16 \times 10 = \mathbf{160}$ Gbps (exact balanced-partition enumeration).
...
\textbf{(c)} Bisection BW $= 8 \times 10 = \mathbf{80}$ Gbps (exact balanced-partition enumeration).
...
\textbf{(c)} Bisection BW $= 16 \times 10 = \mathbf{160}$ Gbps (exact enumeration).
...
We go beyond parts (a)--(c) with three experiments that reveal insights not obtainable from lecture material alone. All numbers are generated by our analysis engine and can be reproduced by clicking the \textbf{Run All Analysis} button in the visualizer, which cycles through every topology and displays the full results in a collapsible panel.
...
(1)~For the given $k{=}2$, $d{=}3$ diagram: 1 path per root switch (4 roots), 2 M1$\to$M3 paths, 4 M1$\to$M5 paths (1 via S2), bisection BW of 80\,Gbps.
...
All links are assumed to be 10\,Gbps. In practice, spine links run at higher speeds (40/100\,Gbps) and Jupiter's OCS dynamically reconfigures bandwidth. Our exact bisection algorithm is exponential in host groups; for $>$20 groups the code falls back to a spatial heuristic.

The source code is in the appendix and runs live in the browser. All statistics reported in this paper can be regenerated via the \textbf{Run All Analysis} button. The original visualizer is at \url{https://github.com/h8liu/ftree-vis}.
